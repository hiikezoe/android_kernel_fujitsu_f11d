...
static inline int rfcomm_check_security(struct rfcomm_dlc *d)
{
	...
	case BT_SECURITY_HIGH:
		BT_DBG(" _sec__  BT_SECURITY_HIGH");			//WLAN/BT
		auth_type = HCI_AT_GENERAL_BONDING_MITM;
		auth_type = HCI_AT_GENERAL_BONDING;				//WLAN/BT
		d->sec_level = BT_SECURITY_MEDIUM;
		break;
	case BT_SECURITY_MEDIUM:
		BT_DBG(" _sec__  BT_SECURITY_MEDIUM");			//WLAN/BT
		auth_type = HCI_AT_GENERAL_BONDING;
		break;
	default:
		BT_DBG(" _sec__  BT_SECURITY_else");			//WLAN/BT
...
/* FUJITSU:2012-03-23 WLAN/BT delete start */
#if 0
...
#endif
/* FUJITSU:2012-03-23 WLAN/BT delete end */
...
static int __rfcomm_dlc_open(struct rfcomm_dlc *d, bdaddr_t *src, bdaddr_t *dst, u8 channel)
{
	...
	int err = 0;
	int ret = 0; // FUJITSU WLAN/BT 2012/06/06 add
	int i = 0; // FUJITSU WLAN/BT 2012/06/06 add
	...
	BT_DBG("%s called", __func__); // FUJITSU WLAN/BT 2012/06/06 add
	...
	if (!s) {
/* FUJITSU WLAN/BT 2012/06/06  start */
...
/* FUJITSU WLAN/BT 2012/06/06  end */
...
#if 0 /* FUJITSU:2011-09-07 WLAN/BT delete start */
					rfcomm_dlc_lock(d);
					d->state = BT_CONNECT2;
					d->state_change(d, 0);
					rfcomm_dlc_unlock(d);
#endif /* FUJITSU:2011-09-07 WLAN/BT delete end */
				} else
					rfcomm_dlc_accept(d);
			}
			continue;
	...
	list_for_each_safe(p, n, &s->dlcs) {
		...
		if (d->state == BT_CONNECTED && !status && encrypt == 0x00) {
			if (d->sec_level == BT_SECURITY_MEDIUM) {
				...
				BT_DBG(" _sec__ BT_SECURITY_HIGH");		//WLAN/BT
				d->sec_level = BT_SECURITY_MEDIUM;		//WLAN/BT
				__rfcomm_dlc_close(d, ECONNREFUSED);
				continue;
			}
		}
