...
int hw_id;
int open_state;
static struct wake_lock wake_lock_s;
static struct wake_lock wake_lock_i;
#endif

extern void set_oncall(int value);
...
#define	CONFIG_GPIO_IS3
#ifdef	CONFIG_GPIO_IS3
#define	CONFIG_GPIO_APO
#define	DSP_POWER_CORE	"gp15"
#define	GP_CLK_SHARE
#endif
...
/*	GPIO, XRESET LOW, CLKIN 1 cycle	*/
	...
	if (hw_id < 4)
	{
		wake_lock(&wake_lock_s);
		wake_lock(&wake_lock_i);
	}else
		set_oncall(1); //enable proximity INT mode
#endif
...
/*	release GPIO */
	...
	if (hw_id < 4)
	{
		wake_unlock(&wake_lock_s);
		wake_unlock(&wake_lock_i);
	}else
		set_oncall(0); //disable proximity INT mode
#endif
...
static int wm0010_resume(struct spi_device *spi)
{
	...
	/* setup GPIO settings */
		...
		ret = vreg_set_level(pdata->vreg_core, 1100);	/* core 1.1V */
		if (ret) {
			dev_err(&spi->dev, "vreg_set_level(CORE, 1100) = %d\n", ret);
		}
		ret = vreg_enable(pdata->vreg_core);
		if (ret) {
...
#if defined(T_QCI_IS3)
static ssize_t wm0010_state(struct device *dev, struct device_attribute *attr, char *buf)
{
	int state;
	int w_switch;

	state = open_state;
	w_switch = gpio_get_value(126);

	return sprintf(buf, "%d; switch = %d\n", state, w_switch);
}

static DEVICE_ATTR(state,  S_IALLUGO , wm0010_state , NULL);

static struct attribute *wm0010_attrs[] = {
	&dev_attr_state.attr,
	NULL,
};
static struct attribute_group wm0010_attr_group = {
	.attrs = wm0010_attrs,
};
#endif
...
static int __devinit wm0010_spi_probe(struct spi_device *spi)
{
	...
	wake_lock_init(&wake_lock_s, WAKE_LOCK_SUSPEND, "wolfson_s");
	wake_lock_init(&wake_lock_i, WAKE_LOCK_IDLE, "wolfson_i");
#endif
...
#if defined(T_QCI_IS3)
	ret = sysfs_create_group(&spi->dev.kobj, &wm0010_attr_group);
	if (ret)
