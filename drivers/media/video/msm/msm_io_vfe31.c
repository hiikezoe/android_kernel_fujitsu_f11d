...
#ifdef T_QCI_IS3
#define	MIPI_PHY_D1_CONTROL_HS_REC_EQ_SHFT				0x1c	
#endif
...
#if defined(T_QCI_IS3) && defined(CONFIG_OV5640_IS3)
...
static void msm_camera_vreg_disable(void)
{
	...
	gpio_set_value(2,0);
	gpio_set_value(3,0);
	gpio_set_value(121,0);
	gpio_tlmm_config( GPIO_CFG(2, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_DISABLE );
	gpio_tlmm_config( GPIO_CFG(3, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_DISABLE );
	gpio_tlmm_config( GPIO_CFG(121, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_DISABLE );
}
...
int msm_camio_csi_config(struct msm_camera_csi_params *csi_params)
{
	...
	for (i = 0; i < csi_params->lane_cnt; i++)
		...
		#ifdef T_QCI_IS3
		val = (0x1F << MIPI_PHY_CL_CONTROL_HS_TERM_IMP_SHFT) |
	...
	#ifdef T_QCI_IS3
	val = 0x10 << MIPI_PHY_D0_CONTROL_HS_REC_EQ_SHFT;
	CDBG("%s MIPI_PHY_D0_CONTROL val=0x%x\n", __func__, val);		
	#else
	...
	val = (0x1 << MIPI_PHY_D1_CONTROL_MIPI_CLK_PHY_SHUTDOWNB_SHFT) |
		...
		(0x10 << MIPI_PHY_D1_CONTROL_HS_REC_EQ_SHFT);	
	#else
	val = (0x1 << MIPI_PHY_D1_CONTROL_MIPI_CLK_PHY_SHUTDOWNB_SHFT) |
		(0x1 << MIPI_PHY_D1_CONTROL_MIPI_DATA_PHY_SHUTDOWNB_SHFT);
	#endif
