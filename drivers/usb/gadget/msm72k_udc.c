...
static inline enum chg_type usb_get_chg_type(struct usb_info *ui)
{
      ...
      u32 data;
      enum chg_type now_chg;
      struct msm_otg *otg = to_msm_otg(ui->xceiv);

      data = readl(USB_PORTSC) & PORTSC_LS;
      printk(KERN_ERR "[USB] usb_get_chg_type(PORTSC_LS) = 0x%x\n",data);
      if (data == 0)
      {
            now_chg = USB_CHG_TYPE__SDP;
      }
      else if (data == PORTSC_LS)
      {
            now_chg = USB_CHG_TYPE__WALLCHARGER;
      }
      else
      {
            data = readl(USB_PORTSC) & PORTSC_LS;
            printk(KERN_ERR "[USB] usb_get_chg_type_2(PORTSC_LS) = 0x%x\n",data);
            if (data == PORTSC_LS)
            {
                now_chg = USB_CHG_TYPE__WALLCHARGER;
            }
      ...
      otg->pdata->qci_chg_connected(now_chg);

      printk(KERN_ERR "[USB] qci_offline_charging_status()=%d\n",otg->pdata->qci_offline_charging_status());
      if (otg->pdata->qci_offline_charging_status() == 1)
      {
          now_chg = USB_CHG_TYPE__WALLCHARGER;
      }
