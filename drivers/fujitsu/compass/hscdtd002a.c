...
/*
static ssize_t compass_wia_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	uint8_t buffer[1];
	hscd_read_regs(HSCD_WIA,buffer,1);
	return sprintf(buf, " who I am: 0x%p\n",buffer);
}

static ssize_t compass_data_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	compass_data_t *measurement;
	int ret = 0;
	ret = hscd_ioctl_measure(measurement);
	return sprintf(buf,"[ x, y, z ]: [ %d, %d, %d ], ret: %d\n",measurement->magnetism[0],measurement->magnetism[1],measurement->magnetism[2],ret);
}

static DEVICE_ATTR(data, S_IRUGO, compass_wia_show, NULL);
static DEVICE_ATTR(wia, S_IRUGO, compass_data_show, NULL);
...
static int hscd_setmode(uint8_t mode)
{
	...
	if (mode == HSCD_MODE_STANBY_V2 || mode == HSCD_MODE_STANBY_V3) {
		...
		} else if (hscd->measurement.measurement_range == HSCD_MEASUREMENT_RANGE_13) {
...
/*	ret = sysfs_create_group(&hscd->reg.st_client->dev.kobj, &sysfs_attribute_group);
	if (ret)
...
//	sysfs_remove_group(&hscd->reg.st_client->dev.kobj, &sysfs_attribute_group);
}
