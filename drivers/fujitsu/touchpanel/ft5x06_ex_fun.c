...
static unsigned char CTPM_FW[30000];
int FW_length = 0;
static unsigned char CTPM_FW_5507[]=
{
	#include "ft_app_0x55_0x07.i"
};
static unsigned char CTPM_FW_5538[]=
{
	#include "ft_app_0x55_0x38.i"
};
static unsigned char CTPM_FW_5583[]=
{
	#include "ft_app_0x55_0x83.i"
};

static unsigned char CTPM_FW_AA07[]=
{
	#include "ft_app_0xaa_0x07.i"
};
static unsigned char CTPM_FW_AA38[]=
{
	#include "ft_app_0xaa_0x38.i"
};

static unsigned char CTPM_FW_A507[]=
{
	#include "ft_app_0xa5_0x07.i"
};
#ifdef CTPM_DOWNLOAD
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if((CTPM_FW[fw_len-9]^CTPM_FW[fw_len-7])==0xFF
		&& (CTPM_FW[fw_len-8]^CTPM_FW[fw_len-6])==0xFF
		&& (CTPM_FW[fw_len-4]^CTPM_FW[fw_len-5])==0xFF)
	{
		//=========FW upgrade========================*/
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret =  fts_ctpm_fw_upgrade(pbt_buf,FW_length);
		if (i_ret != 0)
		{
...
{
    unsigned int ui_sz;
    int vender_id = get_vender_id();
    int color_lens = get_color_lens();

    if( (vender_id != 0x55) && (vender_id != 0xAA) && (vender_id != 0xA5))
		vender_id = get_nv_vender_id();
   if( (color_lens != 0x07) && (color_lens != 0x38) && (color_lens != 0x83))
		color_lens = get_nv_color_lens();

    if(vender_id == 0x55){
		if(color_lens == 0x07){
			FW_length = sizeof(CTPM_FW_5507);
			memcpy(CTPM_FW, CTPM_FW_5507,FW_length);
			printk(" == 5507: %d ==\n",FW_length);
		}else if(color_lens == 0x38){
			FW_length = sizeof(CTPM_FW_5538);
			memcpy(CTPM_FW,CTPM_FW_5538,FW_length);
			printk(" == 5538: %d ==\n",FW_length);
		}else if(color_lens == 0x83){
			FW_length = sizeof(CTPM_FW_5583);
			memcpy(CTPM_FW,CTPM_FW_5583,FW_length);
			printk(" == 5583: %d ==\n",FW_length);
		}
    }else if(vender_id == 0xAA){
		if(color_lens == 0x07){
			FW_length = sizeof(CTPM_FW_AA07);
			memcpy(CTPM_FW,CTPM_FW_AA07,FW_length);
			printk(" == AA07: %d ==\n",FW_length);
		}else if(color_lens == 0x38){
			FW_length = sizeof(CTPM_FW_AA38);
			memcpy(CTPM_FW,CTPM_FW_AA38,FW_length);
			printk(" == AA38: %d ==\n",FW_length);
		}else{
			FW_length = sizeof(CTPM_FW_AA07);
			memcpy(CTPM_FW,CTPM_FW_AA07,FW_length);
    ...
    }else if(vender_id == 0xA5){
		if(color_lens == 0x07){
			FW_length = sizeof(CTPM_FW_A507);
			memcpy(CTPM_FW,CTPM_FW_A507,FW_length);
			printk(" == A507: %d ==\n",FW_length);
		}
    }
    ui_sz = FW_length;
...
printk("%s: %d\n",__func__,__LINE__);
	...
	printk("%s: %d, reg_val[0]: 0x%x, reg_val[1]: 0x%x\n",__func__,__LINE__,reg_val[0],reg_val[1]);
...
int fts_ctpm(void)
{
	...
	printk("%s: %d  uc_tp_fm_ver: 0x%x\n",__func__,__LINE__,uc_tp_fm_ver);
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	printk("%s: %d  uc_host_fm_ver: 0x%x\n",__func__,__LINE__,uc_host_fm_ver);
...
//    	printk("[FTS] Step 1: Reset CTPM test\n");
	...
	gpio_direction_output(TS_RESET, 0);
	msleep(5);
	gpio_direction_output(TS_RESET, 1);
	printk("[FTS] Step 1: Reset TS_RESET\n");
...
static u8 ft5x0x_enter_factory(struct ft5x0x_ts_data *ft5x0x_ts)
{
	...
	printk("+++ %s: %d,  ft5x0x_ts->irq: %d\n",__func__,__LINE__,ft5x0x_ts->irq);
...
int fts_ctpm_fw_upgrade_with_app_file(char * firmware_name)
{
	...
	if((pbt_buf[fwsize-9]^pbt_buf[fwsize-7])==0xFF
		&& (pbt_buf[fwsize-8]^pbt_buf[fwsize-6])==0xFF
		&& (pbt_buf[fwsize-4]^pbt_buf[fwsize-5])==0xFF)
	{
		/*call the upgrade function*/
