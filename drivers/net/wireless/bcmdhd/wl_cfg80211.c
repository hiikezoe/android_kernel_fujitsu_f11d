...
#if 1 // 20120614 AP-WPS2.0 4.2.13 fix
#define WPS_ID_VENDOR_EXT         0x1049
#define WPS_ID_WPS_STATE          0x1044
#define WPS_ID_AP_SETUP_LOCKED    0x1057
#define WPS_ID_SELECTED_REGISTRAR   0x1041
#endif// 20120614 AP-WPS2.0 4.2.13 fix
		...
		if (subelt_id == WPS_ID_VERSION) {
			...
			*pbc = (HTON16(val) == DEV_PW_PUSHBUTTON) ? true : false;
			...
			WL_DBG(("  attr WPS_ID_SELECTED_REGISTRAR_CONFIG_METHODS"
				": cat=%u\n", HTON16(val)));
#if 1 // 20120614 AP-WPS2.0 4.2.13 fix
		} else if (subelt_id == WPS_ID_VENDOR_EXT) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_DBG(("  attr WPS_ID_VENDOR_EXT= %u\n", HTON16(val)));
		} else if (subelt_id == WPS_ID_WPS_STATE) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_DBG(("  attr WPS_ID_WPS_STATE= %u\n", HTON16(val)));
		} else if (subelt_id == WPS_ID_AP_SETUP_LOCKED) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_DBG(("  attr WPS_ID_AP_SETUP_LOCKED= %u\n", HTON16(val)));
		} else if (subelt_id == WPS_ID_SELECTED_REGISTRAR) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_DBG(("  attr WPS_ID_SELECTED_REGISTRAR= %u\n", HTON16(val)));
#endif// 20120614 AP-WPS2.0 4.2.13 fix
		} else {
	...
	struct vif_params *params)
{
	...
	case NL80211_IFTYPE_MONITOR:
		WL_ERR(("NL80211_IFTYPE_MONITOR\n"));
		return wl_cfg80211_add_monitor_if(name);
	case NL80211_IFTYPE_P2P_CLIENT:
		WL_ERR(("NL80211_IFTYPE_P2P_CLIENT\n"));
	case NL80211_IFTYPE_STATION:
		wlif_type = WL_P2P_IF_CLIENT;
		mode = WL_MODE_BSS;
		break;
	case NL80211_IFTYPE_P2P_GO:
		WL_ERR(("NL80211_IFTYPE_P2P_GO\n"));
	...
	struct key_params *params)
{
	...
	memcpy(key.data, params->key, key.len);
#if 1 // 20120516 for multiple WEP key
	if(   ( params->cipher == WLAN_CIPHER_SUITE_WEP40 )
		||( params->cipher == WLAN_CIPHER_SUITE_WEP104) ){
		// note: In case of WEP, 'primary' is set in wl_cfg80211_config_default_key()
		WL_DBG(("skip 'key.flags = WL_PRIMARY_KEY' (algo=0x%04x)\n", params->cipher ));
	}else{
		key.flags = WL_PRIMARY_KEY;
		WL_DBG(("set  'key.flags = WL_PRIMARY_KEY' (algo=0x%04x)\n", params->cipher ));
	}
#else // 20120516 for multiple WEP key
	key.flags = WL_PRIMARY_KEY;
#endif// 20120516 for multiple WEP key
	switch (params->cipher) {
	case WLAN_CIPHER_SUITE_WEP40:
		key.algo = CRYPTO_ALGO_WEP1;
		val = WEP_ENABLED;
	...
	const u8* buf, size_t len, u64 *cookie)
{
	...
	*cookie = id;
		...
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			if ((wps_ie = wl_cfgp2p_find_wpsie((u8 *)(buf + ie_offset), ie_len))
				...
				wpsie_len = wps_ie->length + sizeof(wps_ie->length) +
					sizeof(wps_ie->tag);
				WL_ERR(("call wl_cfgp2p_set_management_ie()\n"));
	...
	WL_ERR((" da=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n",
		action_frame->da.octet[0], action_frame->da.octet[1], action_frame->da.octet[2],action_frame->da.octet[3], action_frame->da.octet[4], action_frame->da.octet[5]));
	...
	WL_ERR(("action_frame->len: %d chan %d category %d subtype %d\n",
	...
	u16 frame_type, bool reg)
{
	...
	WL_ERR(("**: frame_type: %x, reg: %d addr:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n",  frame_type, reg,
	...
	struct beacon_parameters *info)
{
	...
	if (p2p_is_on(wl) &&
		...
		if ((ssid_ie = bcm_parse_tlvs((u8 *)&info->head[ie_offset],
			...
			if ((wps_ie = wl_cfgp2p_find_wpsie((u8 *)info->tail,
				...
				if ( (wl->ap_info) && (wl->ap_info->wps_ie) ) {
					WL_DBG(("Clear wl->ap_info->wps_ie\n"));
					kfree(wl->ap_info->wps_ie);
					wl->ap_info->wps_ie = NULL;
				}
				if ( (wl->ap_info) && (wl->ap_info->beacon_ie_len > 0) ) {
					WL_DBG(("Clear wl->ap_info->beacon_ie\n"));
				...
				if ( (wl->ap_info) && (wl->ap_info->probe_res_ie_len > 0) ) {
					WL_DBG(("Clear wl->ap_info->probe_res_ie\n"));
				...
				if ( (wl->ap_info) && (wl->ap_info->assoc_res_ie_len > 0) ) {
					WL_DBG(("Clear wl->ap_info->assoc_res_ie\n"));
		...
		if ((wps_ie = wl_cfgp2p_find_wpsie((u8 *)info->tail, info->tail_len)) != NULL) {
			...
			WL_DBG(("sec %s->* / wpa_ie %s->%s / wpa2_ie %s->%s\n",
				(wl->ap_info->security_mode)?"true":"false",
				(wl->ap_info->wpa_ie == NULL)?"NULL":"noNULL",
				(wpa_ie == NULL)?"NULL":"noNULL",
				(wl->ap_info->rsn_ie == NULL)?"NULL":"noNULL",
				(wpa2_ie == NULL)?"NULL":"noNULL"));
...
#if 0 // 20120514 only 2GHz for FJ
	wdev->wiphy->bands[IEEE80211_BAND_5GHZ] = NULL;
	WL_ERR(("wdev->wiphy->bands[IEEE80211_BAND_5GHZ] is set NULL !!\n"));
#else // 20120514 only 2GHz for FJ
	wdev->wiphy->bands[IEEE80211_BAND_5GHZ] = &__wl_band_5ghz_a;
#endif// 20120514 only 2GHz for FJ
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		mgmt_frame += 6; // shift 6 bytes
		printk("force mgmt_frame[0]=0x40\n");
		mgmt_frame[0]=0x40;
#endif// 20120614 AP-WPS2.0 4.2.13 fix
	} else {
		mgmt_frame = (u8 *)((wl_event_rx_frame_data_t *)rxframe + 1);
...
s32 wl_update_wiphybands(struct wl_priv *wl)
{
	...
	for (; *phy; phy++) {
		if (*phy == 'a'){
			WL_ERR(("wl_update_wiphybands(): phy = 'a'\n"));
		}else if (*phy == 'n'){
			WL_ERR(("wl_update_wiphybands(): phy = 'n'\n"));
		}else if (*phy == 'b'){
			WL_ERR(("wl_update_wiphybands(): phy = 'b'\n"));
		}else if (*phy == 'g'){
			WL_ERR(("wl_update_wiphybands(): phy = 'g'\n"));
		}else{
			WL_ERR(("wl_update_wiphybands(): phy = 'other'\n"));
		}
		if (*phy == 'a' || *phy == 'n') {
			wiphy = wl_to_wiphy(wl);
	...
	enum wl_management_type type)
{
	struct wl_priv *wl;
	struct net_device *ndev = NULL;
#if 1  // 20120530 WFA 4.1.3 debug
	struct ether_addr primary_mac;
#endif // 20120530 WFA 4.1.3 debug
	s32 ret = 0;
	s32 bssidx = 0;
	s32 pktflag = 0;
	wl = wlcfg_drv_priv;
	if (wl->p2p && wl->p2p->vif_created) {
		...
		if (!wl->p2p->on) {
			...
			*/
			...
			if (unlikely(ret)) {
				//goto exit;
				WL_ERR(("wl_cfgp2p_enable_discovery() is fail.\n"));
				return ret;
			}
		}
...
#if 0 // 20120518 WFA 4.1.3 debug
	else{
		if( type == WL_PROBE_RESP ){
			ndev = wl_to_p2p_bss_ndev(wl, P2PAPI_BSSCFG_CONNECTION);
			bssidx = wl_to_p2p_bss_bssidx(wl, P2PAPI_BSSCFG_CONNECTION);
			if (ndev != NULL){
				WL_ERR(("WFA 4.1.3 debug !!!! \n"));
				ret = wl_cfgp2p_set_management_ie(wl, ndev, bssidx, VNDR_IE_PRBRSP_FLAG, buf, len);
			}
		}
	}
#endif// 20120518 WFA 4.1.3 debug
	return ret;
}
