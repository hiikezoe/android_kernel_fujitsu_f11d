...
static int msm_fb_resume_sub(struct msm_fb_data_type *mfd)
{
 ...
 #if defined(T_QCI_IS3)
static ssize_t msm_fb_off(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct fb_info *fbi = dev_get_drvdata(dev);
	...
	msm_fb_suspend_sub(mfd);
       ret = snprintf(buf, PAGE_SIZE, "msm_fb_off\n");
	return ret;
}
static ssize_t msm_fb_on(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct fb_info *fbi = dev_get_drvdata(dev);
	...
	msm_fb_resume_sub(mfd);
       ret = snprintf(buf, PAGE_SIZE, "msm_fb_on\n");
	return ret;
}
#endif
...
static DEVICE_ATTR(msm_fb_type, S_IRUGO, msm_fb_msm_fb_type, NULL);
 #if defined(T_QCI_IS3)
static DEVICE_ATTR(msm_fb_off, S_IRUGO, msm_fb_off, NULL);
static DEVICE_ATTR(msm_fb_on, S_IRUGO, msm_fb_on, NULL);
#endif

static struct attribute *msm_fb_attrs[] = {
	&dev_attr_msm_fb_type.attr,
#if defined(T_QCI_IS3)
	&dev_attr_msm_fb_off.attr,
	&dev_attr_msm_fb_on.attr,
#endif
	NULL,
};
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	var->height = 80,	/* height of picture in mm, Fujitsu IS3 workaround */
	var->width = 48,	/* width of picture in mm, Fujitsu IS3 workaround */
