...
void qci_chg_connected(enum chg_type chgtype)
{
       if (chgtype == USB_CHG_TYPE__SDP ||
	    chgtype == USB_CHG_TYPE__CARKIT)
       {
            smb329b_vbus_draw(500);
	}
	else if (chgtype == USB_CHG_TYPE__WALLCHARGER)
	{
	     smb329b_vbus_draw(1000);
	}
	else
	{
	    smb329b_vbus_draw(0);
...
EXPORT_SYMBOL(qci_chg_connected);
int qci_offline_charging_status(void)
{
    return qci_get_offline_charging();
}
EXPORT_SYMBOL(qci_offline_charging_status);
#endif
#endif
