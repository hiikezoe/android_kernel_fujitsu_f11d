...
extern void reboot_reset_ram_console(void);
#endif
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	...
	if ((code == SYS_RESTART) && _cmd) {
		...
		} else if (!strcmp(cmd, "SDdownload")) {
			restart_reason = 0x77665510;

#endif
#if defined(T_QCI_IS3)
             } else if (!strcmp(cmd, "novibrator")) {
			restart_reason = 0x77665598;
	      } else if (!strcmp(cmd, "offnoboot")) {
			restart_reason = 0x77665599;
	      } else if (!strcmp(cmd, "offcharge")) {
			restart_reason = 0x77665600;
#endif
